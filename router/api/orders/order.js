...
router.get("/",auth,async(req,res)=>{
  ...
  const status=req.query.status;
    ...
    const orders=await Order.find({"userId":req.user.id,"status":status}).populate({path:"items",populate: { path: "productId", select: ["name", "price"] }}).populate({path:"userId",select: ["fistname", "lastname","email"]}).sort({"createdAt": -1 });
    if(orders.length===0)
    {
